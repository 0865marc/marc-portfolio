...
asset=$(find "$dist/assets" -type f | head -n1);asset=${asset#"$dist"};curl -sSI "http://127.0.0.1:18088$asset"|grep -Eqi 'cache-control:.*public.*immutable';curl -sSI http://127.0.0.1:18088/index.html|grep -Eqi 'cache-control:.*no-cache';curl -sSI http://127.0.0.1:18088/blog/index.html|grep -Eqi 'cache-control:.*no-cache';echo 'isolated nginx cache-header matrix passed'
