...
A merge or push to `origin/main` may verify and publish a GHCR candidate, but it never deploys production. Production requires a distinct release run in `ready_for_release` with a single-use approval bound to the exact commit, immutable image digest, production target, and `ghcr_digest_promotion` trigger. The private deployer and host configuration are intentionally outside this public repository. Inferred permission, force-push, reset, clean, and stash remain forbidden. Post-deploy evidence is required before deployed closure.
