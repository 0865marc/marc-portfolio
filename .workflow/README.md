...
This directory defines prospective, project-relative execution records. It does not migrate or assert the truth of retired task history. `.workflow/` holds per-run requests, plans, state, immutable approvals, evidence, results, and archives; reviewed reusable knowledge belongs in `.agents/` through a pending knowledge delta and independent review. Never hand-edit `.agents/generated/`.

## Run lifecycle

Create an immutable UTC run ID (`YYYYMMDDTHHMMSSZ-short-kebab-slug`) and copy the templates into `.workflow/runs/<run-id>/`. Record discovery and planning, calculate the exact `plan.md` SHA-256, then obtain an attributable immutable plan approval. Set `.workflow/current.yaml` only when the run becomes active. Exactly one non-terminal run may exist.
