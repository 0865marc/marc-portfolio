...
The GitHub repository webhook was deleted on 2026-07-23. A push or merge to `main` no longer invokes the legacy static production deployment described in `ops/github-webhook-autodeploy.md`.

Merging to `main` now verifies the exact commit and publishes a candidate; it does not promote that candidate to production. Production promotion remains deliberately unconfigured. The next migration step is to make the host deployer accept an explicitly approved image digest (or a static artifact SHA), with a separate production approval and post-deploy verification.
