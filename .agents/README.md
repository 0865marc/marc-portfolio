...
`.agents/` is the project-local, reviewable knowledge layer for agents working in this repository. It holds curated context, a formal visual specification, accepted decisions, and bounded knowledge-delta records. CodeGraph provides the separate structural code index. Neither layer replaces application source, Git history, `.workflow/policy.yaml`, or private production policy.
...
`npm run verify` synchronizes CodeGraph before running the authored-knowledge, workflow, application, static-output, and asset gates. The CLI is pinned as a development dependency so indexing and shell queries work after `npm ci`.
